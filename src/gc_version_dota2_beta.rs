...
pub struct GcVersionDota2Beta<'a> {
    api: &'a mut crate::WebApi,
}

impl<'a> GcVersionDota2Beta<'a> {
    ...
    pub async fn get_server_version(&'a mut self) -> Result<get_server_version::Response> {
        ...
        let data = self
            .api
            .request(&format!("IGCVersion_205790/GetServerVersion/v1/{}", &query))
            .await?;
    ...
    pub async fn get_client_version(&'a mut self) -> Result<get_client_version::Response> {
        ...
        let data = self
            .api
            .request(&format!("IGCVersion_205790/GetClientVersion/v1/{}", &query))
            .await?;
