...
pub struct MatchDota2Beta<'a> {
    api: &'a mut crate::WebApi,
}

impl<'a> MatchDota2Beta<'a> {
    ...
    pub async fn get_match_details(
        ...
        let query = format!("?key={}&match_id={}", self.api.get_key()?, match_id);
        let data = self
            .api
    ...
    pub async fn get_league_listing(&'a mut self) -> Result<get_league_listing::Response> {
